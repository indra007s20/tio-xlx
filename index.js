...
const config = {
    A187: '📍BOT DO TIO PAIN📍',
    instagram: '@tio_pain_xlx',
    número: 'wa.me/11949545217',
    ativo: '1 JAN',
    whatsapp: 'https://chat.whatsapp.com/EU1oYYBoBQvLOn3FFeYoPM,
    Tempo: time
}
       ...
       case 'twt':
           tweetdl(value)
               .then(data => {
                    ...
                    let hasil = `Funciona! silahkan klik link di bawah untuk mendownload hasilnya!\nKlik link dibawah🗡️\n\nSize: ${size}\n\nLink: ${link}`
                    client.sendMessage(id, '[ESPERE UM POUCO...', MessageType.text)
                    client.sendMessage(id, hasil ,MessageType.text)
               })
               .catch(err => {
       ...
       case 'tiktok':
           tiktokdl(value)
               .then(data => {
                    ...
                    let hasil = `achado com Sucesso !!! Clique no link abaixo para baixar os resultados! \nClique no link abaixo🗡️\n\nTítulo: ${deskripsi} \n\nDurasi: ${durasi}\n\nNome: ${nama}\n\nUrl: ${url}`;
                    client.sendMessage(id, hasil, MessageType.text) ou
               })
               .catch(err => {
                   console.log(err)
               })
       ...
       case 'foto':
           switch (value) {
               ...
               case 'menino':
                   ...
                   break
               default:
                   client.sendMessage(id, 'Repita com! Pict meninas / meninos \ n \ nPor exemplo : !foto menino', MessageType.text)
                   break
           }
           break
       case 'animepict':
       ...
       case 'igstalk':
           igStalk(value)
               .then(data => {
                   ...
                   client.sendMessage(id, 'estamos fazendo aguarde!', MessageType.text)
      ...
      case 'stickergif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '🐊Aguarde um estante estamos fazendo seu sticker🐊', id)
